...
class Zlibrary:
    ...
    def getBookComments(
        self, bookid: [int, str], hashid: str, page: int = 1, limit: int = 20
    ) -> dict[str, str]:
        """Fetches real Z-Library user comments for a book."""
        return self.__makeGetRequest(
            f"/eapi/book/{bookid}/{hashid}/comments",
    ...
    def __getImageData(self, url: str) -> requests.Response.content:
        res = requests.get(url, headers=self.__headers, proxies=self.proxies)
    ...
    def __getBookFile(self, bookid: [int, str], hashid: str) -> [(str, bytes), None]:
        ...
        if not response or not response.get("success", True):
            error_msg = response.get("message", "获取下载链接失败")
        ...
        res = requests.get(ddl, headers=headers, proxies=self.proxies)
        if res.status_code == 200:
            return filename, res.content
        else:
            raise Exception(f"下载文件失败，HTTP 状态码: {res.status_code}")
