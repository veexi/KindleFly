...
> 📬 **一款高颜值、全自动的 Kindle 电子书本地推送工具 (v1.0.1)**  
> 支持 Windows 系统托盘后台静默运行、定时扫描指定目录、多邮箱服务商预设安全推送（内置 Gmail、QQ 邮箱、网易 163 邮箱、自定义 SMTP）以及 MD5 文件防重复防漏推送保护。
...
*   🔒 **多发信邮箱预设支持**：内置 Gmail、QQ 邮箱、网易 163 邮箱常用参数，支持一键连接测试，并配备保姆级图文申请授权码指南。
*   🌐 **网络代理集成**：内置 Clash 自定义 SOCKS5 / HTTP 代理路由配置，完美解决国内直连 Gmail SMTP 容易连接超时的问题。
...
3.  在 **「已认可的个人文档电子邮箱列表 (Approved Personal Document E-mail List)」** 下，点击「添加新的认可电子邮箱」，将您的发信邮箱（如 `yourname@qq.com` 或 `yourname@gmail.com`）添加进去。
4.  *顺便在此页面记下您 Kindle 设备的专属接收邮箱（如 `xxx@kindle.com` 或 `xxx@free.kindle.com`）。*

### 第二步：申请并获得 16 位发信授权码 (Auth Code)

为了邮箱账号安全，各大主流邮箱服务商已关闭传统登录密码的第三方客户端直连，您必须生成专属的 16 位发信授权码或应用密码：
*   **QQ 邮箱**：进入网页版 QQ 邮箱 ->【设置】->【账户】-> 开启【POP3/SMTP服务】并按提示获取 16 位发信授权码。
*   **网易 163 邮箱**：进入网页版 163 邮箱 ->【设置】->【POP3/SMTP/IMAP】-> 开启【POP3/SMTP服务】并设置授权密码。
*   **谷歌 Gmail**：在谷歌账号安全性里开启「两步验证」，并搜索生成 16 位的「应用专用密码」。

### 第三步：运行与配置 KindleFly

1.  双击运行 **`KindleFly.exe`**。
2.  进入 **📧 邮件发信配置** 标签页：
    *   在“常用邮箱服务商预设”下拉菜单中选择您要使用的发信箱。
    *   填入发信邮箱账号和刚申请到的 16 位授权码/应用密码。
...
KindleFly **不会收集或上传**您的任何敏感信息。您的发信邮箱及授权码在本地是通过 **Base64** 隐藏编码并以纯本地 JSON (`config.json`) 形式存储在您的软件同级目录下，绝不会经由第三方服务器，保障了您个人发信箱的安全。
